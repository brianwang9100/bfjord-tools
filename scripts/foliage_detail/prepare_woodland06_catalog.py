...
"""Write only v0.6 woodland Unity metadata and family additions; leave shared manifest to integration."""
...
names=['Models/'+n+'.fbx' for n in ('MatureOak_B','SilverBirch_B')]+['Textures/'+n+'.png' for n in ('Woodland06Atlas','Woodland06Normal','Woodland06Mask')]+['woodland06-manifest.json']
for name in names:
 ...
 if path.suffix=='.fbx':text=re.sub(r'guid: [a-f0-9]+','guid: '+guid,model)
  ...
  text=re.sub(r'guid: [a-f0-9]+','guid: '+guid,normal if path.stem=='Woodland06Normal' else texture);text=re.sub(r'maxTextureSize: \d+','maxTextureSize: 2048',text);text=text.replace('alphaIsTransparency: 1','alphaIsTransparency: 0')
  if path.stem=='Woodland06Mask':text=text.replace('sRGBTexture: 1','sRGBTexture: 0')
...
(folder/'woodland06-catalog-additions.json').write_text(json.dumps({'revision':'woodland-06','files':entries},indent=2)+'\n');print(json.dumps({'files':len(entries),'bytes':sum(x['bytes'] for x in entries)}))
