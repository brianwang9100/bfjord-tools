...
Four 512 × 512 linear RGBA masks supplement the existing Poly Haven ground scans. Output lives at `assets/BFjordTools/asset-catalog/Assets/BFjord/TerrainDetail`. `catalog-additions.json` supplies entries for the coordinator to merge into the shared catalog at integration.
...
```sh
python3 scripts/terrain_detail/build_masks.py --cache /tmp/bfjord-terrain-source --fetch
```
