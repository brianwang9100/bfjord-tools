...
| `bwork_water_connected` | `action=refresh-surface`, optional `recipePath` | Rebuilds owned visual swell bounds and optional swash apron without carving Terrain or changing canonical node/reach geography. |
...
| Recipe | `oceanSwashRunupHeight`, `oceanSwashRunupDistance`, `oceanSwashPeriod` | Optional render-only runup amplitude 0–1 m, ocean apron 0–24 m, period 4–16 s. Defaults 0, 0, 8 disable runup for existing recipes. These require `refresh-surface`; appearance-only updates retain installed values. |
...
One material shades the shared surface. UV0 stores world XZ; UV1 stores visual flow direction/scale; UV2 stores base-height and bank-envelope gradients; UV3 stores ocean/lake blend gradients. UV4 stores ocean signed distance, its X/Z gradient and a swash validity marker. Vertex color R pins the ordinary wave envelope at canonical banks, G carries authored foam, B blends ocean waves and A blends lake waves. Shared vertices keep mouth displacement coherent. The shader requires the sandbox camera's current URP depth texture.

Opt-in runup expands only the ocean's **visual** union before triangulation; `Sample`, `SampleForBank`, canonical bounds, routes and Terrain carving retain the original footprint. A signed-distance wave raises and lowers that apron through the actual opaque Terrain, so the depth intersection advances and retreats. It uses the same displacement function and derivatives for position and lighting. Its contact fade remains thin enough to see a shallow wash. The apron is a finite coverage allowance, not a promised wet distance: higher banks occlude it. The runup falls to zero over the outermost 4 m and the outer 2 m feather away; it also tapers toward offshore water. Bounds include the **sum** of the ordinary maximum wave amplitude and runup amplitude. Both runup and apron coverage switch off if depth is unavailable, and zero runup retains the original clipped mesh geometry.

The fidelity presets use an 18 m apron, ±0.55 m runup and an 8 s period. The sandbox's actual beach intersects still water about 11–12 m outside the ocean rectangle near X320, which is why a 6 m apron cannot reach its bank. The default `bwork_water_fidelity preset=medium refreshSurface=true` rebuilds the visual surface and preserves Terrain; an appearance-only call cannot enlarge geometry or its displacement allowance. Surface refresh first checks the installed vertex/index arrays against its receipt, then publishes a fresh generation through the existing rollback transaction. A sloped-beach test covers both advance and retreat; actual screenshot/clip evidence must additionally verify the current scene. Near X320, representative trough/crest/trough times are approximately 1.5, 5.5 and 9.5 seconds.

The visual apron rejects overlap with canonical inland water whose level differs from the ocean by more than 0.025 m, matching the canonical overlap tolerance. This prevents a nearby elevated reach from becoming an artificial water ramp when the ocean expands. Shrink `oceanSwashRunupDistance` or extend the sea-level flat connection collar before retrying; rejection leaves the installed surface and Terrain unchanged.
...
The September 2026 runup pass rechecked [Crest's shorelines/depth documentation](https://crest.readthedocs.io/en/4.22.1/user/shallows-and-shorelines.html), its [MIT public repository](https://github.com/wave-harmonic/crest), and [Catlike Coding's depth-intersection tutorial](https://catlikecoding.com/unity/tutorials/flow/looking-through-water/). These support depth-derived bank contact and shallow attenuation. Crest's maintained public package targets Built-in and requires shader model 4.5; its URP product remains separately offered. No third-party source was copied. The chosen implementation extends the existing URP mesh/depth shader with a bounded analytic effect, avoiding a new solver or extra camera. Physical iPad performance remains unverified.
